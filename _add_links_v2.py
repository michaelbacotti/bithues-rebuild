...
def remove_old_related_guides(content):
    """Remove old 'Related Guides Best...' text that appears as bare words at end of article content."""
    # Pattern: Related Guides followed by link-like words ending before share-bar
    # These appear as inline text in a <p> tag: "Related Guides Best X Y Z"
    patterns = [
        # books-like articles: "Related Guides Best X Browse Y Reviews Book Like Z"
        r'Related Guides\s*(?:Best\s+[^<]{3,40}?\s*)?(?:Browse\s+[^<]{3,40}?\s*)?(?:Reviews\s*)?(?:Books\s+Like\s+[^<]{3,40}?\s*)?(?=</p>\s*<div class="share-bar">)',
        r'Related Guides\s*(?:Best\s+[^<]{3,40}?\s*)?(?:Browse\s+[^<]{3,40}?\s*)?(?:Reviews\s*)?(?:Books\s+Like\s+[^<]{3,40}?\s*)?(?=</p>\s*<hr)',
    ]
...
def add_links_before_sharebar(content, filepath):
    ...
    # Remove old Related Guides text
    content = remove_old_related_guides(content)
    
    # Check for "Continue Reading" block already present
    if '>Continue Reading</h3>' in content:
        return content, False
    
    # Insert before <div class="share-bar">
